...
def intcode_computer(array):
  memory = [0] * 10000
  def _(pos, base):
    intcode = str(memory[pos] + 1000000)
    first = int(intcode[-3])
    ...
    if debug:
      if False:
        print(memory[:12])
        print(f'{pos} {intcode[2:]} {opcode} {first} {second} {third}')
    def _val1():
      val1 = None
      if first == 1:
        val1 = memory[pos+1]
      elif first == 2:
        val1 = memory[base + memory[pos+1]]
      else:
